...
class DrivetrainPoseTelemetry:
    ...
    def update(self, estPose: Pose2d, moduleAngles):
        ...
        self.field.getObject("ModulePoses").setPoses(
            [
                estPose.transformBy(
                    Transform2d(robotToModuleTranslations[0], moduleAngles[0])
                ),
                estPose.transformBy(
                    Transform2d(robotToModuleTranslations[1], moduleAngles[1])
                ),
                estPose.transformBy(
                    Transform2d(robotToModuleTranslations[2], moduleAngles[2])
                ),
                estPose.transformBy(
                    Transform2d(robotToModuleTranslations[3], moduleAngles[3])
                ),
            ]
        )
        ...
        self.field.getObject("curObstaclesFixed").setPoses(
            [Pose2d(x, Rotation2d()) for x in self.fixedObstacles]
        )
        self.field.getObject("curObstaclesFull").setPoses(
            [Pose2d(x, Rotation2d()) for x in self.fullObstacles]
        )
        self.field.getObject("curObstaclesThird").setPoses(
            [Pose2d(x, Rotation2d()) for x in self.thirdObstacles]
        )
        ...
        if trajIn is not None:
            ...
            while sampTime < trajIn.get_total_time():
                ...
                if state is not None:
                    stateList.append(self._choreoToWPIState(state))
                sampTime += sampStep

            # Make sure final pose is in the list
            stateList.append(
                self._choreoToWPIState(flip(transform(trajIn.samples[-1])))
            )
    ...
    def _choreoToWPIState(self, inVal: choreo.trajectory.SwerveSample):
        ...
        return Trajectory.State(
            acceleration=0, pose=inVal.get_pose(), t=inVal.timestamp, velocity=velNet
        )
